...
import cn.kuzuanpa.thinkerMixin.thinkerMixin;
...
public class RenderUtils {
	...
	public final static void renderWorldToTexture(float renderTime, int framebuffer, int width, int height) {
		...
		float[] soundLevelBackup = new float[SoundCategory.values().length];
		for (SoundCategory s :SoundCategory.values())soundLevelBackup[s.getCategoryId()]=mc.gameSettings.getSoundLevel(s);
		...
		try {
			...
			for (SoundCategory s :SoundCategory.values())mc.gameSettings.setSoundLevel(s,soundLevelBackup[s.getCategoryId()]);
